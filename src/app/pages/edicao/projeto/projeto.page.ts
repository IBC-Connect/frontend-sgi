...
export class ProjetoPage implements OnInit {
  ...
  public inicializar() :void{
    ...
    this.projeto.dataInicio = this.projeto.dataInicio.includes('/')? this.projeto.dataInicio : DateUtil.dateFormatterBrazil(this.projeto.dataInicio); 
  }

  public disabledBotaoEditar(): boolean{
    return  !ValidadorCamposObrigatorios.isCampoPreenchido(this.projeto.nome) 
  ...
  public editar(): void {
    if (this.projeto) {
      if (this.projeto.dataInicio && !DateUtil.verificarIsDataValida(this.projeto.dataInicio)) {
  ...
  public verificarHorariosEncontro(){
    ...
    this.projeto.diasEncontro.forEach(element => {
      if(element.horarioInicial && element.horarioFinal){
         if(DateUtil.verificarIntervaloHora(element.horarioInicial, element.horarioFinal)){
           retorno =true;
        }
      }
    });
